...
can also use Gradle. It also shows values of `com.example` and `accesing-data-jpa` as the
...
----
include::complete/src/main/java/com/example/accessingdatajpa/Customer.java[tag=sample]
----
====
...
as the following listing (in `src/main/java/hello/CustomerRepository.java`) shows:
...
{JB: Account for the application class that the Initialzr created.}
